...
logvraissemblance <- function(params){
  ...
  logphi4 <- dweibull(data, shape = params[4], scale = params[5], log = TRUE)
...
resultats <- foreach (i=1:nbre_random_start, .packages=c("DPQ", "nloptr", "VaRES")) %dopar% {
  ...
  t <- try(repeat{
    ...
    logphi4 <- dweibull(data, shape = alpha_r, scale = beta_r, log = TRUE)
    ...
    res_nlopt <- nloptr(x0 = c(mu_r, sigma_r, lambda_r, alpha_r, beta_r), eval_f = logvraissemblance_pour_nloptr, opts = list(algorithm = "NLOPT_LN_BOBYQA", maxeval = 10000, tol_rel=1e-15, xtol_abs=1e-15), lb = c(1, 1, 0.1, 0.1, 0.1))
...
summary(resultats)
saveRDS(resultats, file = "resultats_application_weibull.RData")
################################################################################
##--------------------------------RESULTATS-----------------------------------##
################################################################################

resultats <- readRDS("resultats_application_weibull.RData")
...
ggplot(pop_classe_avant %>% filter(nb_transcrit > 50)%>%filter(nb_transcrit < 30000), aes(x = nb_transcrit)) +
  ...
  scale_color_manual(name = "Distributions",
                     values = c("Normale modélisant les goutteletes à une cellule" = "red",
                                "Normale modélisant les goutteletes à deux cellule" = "darkgreen")) +
  theme(legend.position = "top",
        legend.background = element_rect(fill="white",
                                         size=0.5, linetype="solid",
                                         colour ="#909090")) +
...
normale_weibull <- ggplot(sum_gene_df %>% filter(nb_transcrit > 300)%>%filter(nb_transcrit < 20000), aes(x = nb_transcrit)) +
  ...
  theme_bw()+
  stat_function(aes(color= "Première normale"),size = 1, fun = function(x) {(dnorm(x, mean = mu_hat, sd = sigma_hat)*pi2_hat*149933102)})+
  stat_function(aes(color= "Seconde normale"), size = 1, fun = function(x) {(dnorm(x, mean = 2*mu_hat, sd = sqrt(2)*sigma_hat)*pi3_hat*149933102)})+
  ...
  theme(legend.position = "top",
        legend.background = element_rect(fill="white",
                                         size=0.5, linetype="solid",
                                         colour ="#909090")) +
  theme(legend.position = c(0.73, 0.94),
        legend.direction = "horizontal")+
  theme(legend.title = element_blank())+
  ylab("Proportion") +
  xlab("Nombre de transcrits") +
  labs(title = "Distributions des lois normales superposées aux données réelles", subtitle = "dans le cas d'un modèle de mélange avec la loi de Weibull") +
  theme(plot.title = element_text(face = "bold",size = 13, hjust = 0, vjust = 0), plot.subtitle = element_text(size = 10, hjust = 0))
...
logphi4 <- dweibull(data, shape = alpha_hat, scale = beta_hat, log = TRUE)
