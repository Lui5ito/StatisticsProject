...
logvraissemblance <- function(params){
  logphi1 <- dpois(data, params[3], log = TRUE)+log(1-exp(-params[3]))
...
resultats <- foreach (i=1:nbre_random_start, .packages=c("DPQ", "nloptr")) %dopar% {
  ...
  t <- try(repeat{
    ...
    res_nlopt <- nloptr(x0 = c(mu_r, sigma_r, lambda_r), eval_f = logvraissemblance_pour_nloptr, opts = list(algorithm = "NLOPT_LN_BOBYQA", maxeval = 10000, tol_rel=1e-15, xtol_abs=1e-15), lb = c(1, 1))
