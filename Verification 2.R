...
logvraissemblance <- function(params){
  ...
  logphi2 <- ifelse(data==1, pnorm(data, mean = params[1], sd = params[2], log.p = TRUE) , logspace.sub(pnorm(data+1/2, mean = params[1], sd = params[2], log.p = TRUE), pnorm(data-1/2, mean = params[1], sd = params[2], log.p = TRUE)))
  logphi3 <- ifelse(data==1,pnorm(data, mean = 2*params[1], sd = sqrt(2)*params[2], log.p = TRUE),logspace.sub(pnorm(data+1/2, mean = 2*params[1], sd = sqrt(2)*params[2], log.p = TRUE), pnorm(data-1/2, mean = 2*params[1], sd = sqrt(2)*params[2], log.p = TRUE)) )
...
nbre_de_coeurs_voulu <- 6  ## Il faut que ca soit inférieur au nombre de coeurs disponibles
...
taille_echantillon <- c(10^3)
nbre_repetition <- 6
algorithmes <- c("NLOPT_LN_COBYLA") #### Pas de "NLOPT_LN_SBPLX" car trop proche de L-BFGS-B et difficile de justifier qu'il ne marche pas.  
...
all_results_temp <-foreach (j=1:nbre_repetition, .packages = c("DPQ", "nloptr")) %dopar% {
  ...
  for (i in 1:length(taille_echantillon)) {
    ...
    data <- c(rpois(taille_echantillon_en_cours*0.5, lambda_cible), round(rnorm(taille_echantillon_en_cours*0.25, mu_cible, sigma_cible)), round(rnorm(taille_echantillon_en_cours*0.25, 2*mu_cible, sqrt(2)*sigma_cible)))
    ...
    for (k in 1:length(algorithmes)){
      ...
      for (l in 1:nbre_random_start){
        ...
        t <- try(repeat{
          ...
          temps_ce_tour <- system.time(res_nlopt <- nloptr(x0 = c(mu_r, sigma_r, lambda_r), eval_f = logvraissemblance_pour_nloptr, opts = list(algorithm = algo_en_cours, maxeval = 10000, tol_rel=1e-15, xtol_abs=1e-15), lb = c(1, 1, 0.1)))
        ...
        if(!(inherits(t, "try-error"))){
          ...
          resultats_temp[1, i, k, l] <- logvraissemblance(c(mu_r, sigma_r, lambda_r)) ## log-vraisemblance complétée
