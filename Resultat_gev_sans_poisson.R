...
logvraissemblance <- function(params){
  logphi1 <- dgev(data, loc = params[3], scale = params[4], shape = params[5], log = TRUE)
...
nbre_parametre_interet <- 9 #### log-vraisemblance complétée, mu, sigma, pi1, pi2, pi3, alpha, beta, gamma
...
resultats <- foreach (i=1:nbre_random_start, .packages=c("DPQ", "nloptr", "evd")) %dopar% {
  ...
  suite_beta <- c(beta_r)
  suite_gamma <- c(gamma_r)
  
  t <- try(repeat{
    
    ## On a bseoin de ces calculs pour lambda_r
    logphi1 <- dgev(data, loc = alpha_r, scale = beta_r, shape = gamma_r, log = TRUE)
    ...
    res_nlopt <- nloptr(x0 = c(mu_r, sigma_r, alpha_r, beta_r, gamma_r), eval_f = logvraissemblance_pour_nloptr, opts = list(algorithm = "NLOPT_LN_BOBYQA", maxeval = 10000, tol_rel=1e-15, xtol_abs=1e-15), lb = c(1, 1, 0, 0.01, 0))
    
    ## Mises à jours des paramètres
    mu_r <- res_nlopt$solution[1]
    sigma_r <- res_nlopt$solution[2]
    alpha_r <- res_nlopt$solution[3]
    beta_r <- res_nlopt$solution[4]
    gamma_r <- res_nlopt$solution[5]
    pi1_r <- T1/n
    pi2_r <- T2/n
    pi3_r <- T3/n
    ...
    suite_gamma <- c(suite_gamma, gamma_r)
  ...
  if(!(inherits(t, "try-error"))){
    ...
    Lvc <- c(Lvc, logvraissemblance(c(mu_r, sigma_r, alpha_r, beta_r, gamma_r))) #Pour avoir autant de points que les paramètres
    ma_liste <- list(logvraisemblance  = Lvc, mu = suite_mu, sigma = suite_sigma, pi1 = suite_pi1, pi2 = suite_pi2, pi3 = suite_pi3, alpha = suite_alpha, beta = suite_beta, gamma = suite_gamma)
  }
  return (ma_liste)
}
toc()
...
summary(resultats)
saveRDS(resultats, file = "resultats_application_gev_sans_poisson.RData")
################################################################################
##--------------------------------RESULTATS-----------------------------------##
################################################################################

resultats <- readRDS("resultats_application_gev_sans_poisson.RData")

## On veut maintenant récupérer le meilleur des random starts, celui qui a la log-vraisemblance complétée la plus élevée.
max_index <- 1
length(resultats[[8]])
max_lvc <- tail(resultats[[2]][[1]], 1)
for (i in 3:length(resultats)){
...
## beta
resultats[[max_index]][[8]]
beta <- ggplot(as.data.frame(resultats[[max_index]][[8]]), aes(x=seq_along(resultats[[max_index]][[8]]), y=resultats[[max_index]][[8]])) +
  geom_point(color = "#21ADE5", size = 2.5) +
  theme_bw() +
  xlab("Itération") +
...
beta <- ggplot(as.data.frame(resultats[[max_index]][[9]]), aes(x=seq_along(resultats[[max_index]][[9]]), y=resultats[[max_index]][[9]])) +
  geom_point(color = "#21ADE5", size = 2.5) +
  theme_bw() +
  xlab("Itération") +
...
beta_hat <- tail(resultats[[max_index]][[8]], 1)
gamma_hat <- tail(resultats[[max_index]][[9]], 1)
...
logphi1 <- dgev(data, loc = alpha_hat, scale = beta_hat, shape = gamma_hat, log = TRUE)
