...
logvraissemblance <- function(params){
  ...
  logphi4 <- dgev(data, loc = params[4], scale = params[5], shape = params[6], log = TRUE)
...
nbre_parametre_interet <- 11 #### log-vraisemblance complétée, lambda, mu, sigma, pi1, pi2, pi3, pi4, alpha, beta
...
resultats <- foreach (i=1:nbre_random_start, .packages=c("DPQ", "nloptr", "evd")) %dopar% {
  ...
  t <- try(repeat{
    ...
    logphi4 <- dgev(data, loc = alpha_r, scale = beta_r, shape = gamma_r, log = TRUE)
    ...
    res_nlopt <- nloptr(x0 = c(mu_r, sigma_r, lambda_r, alpha_r, beta_r, gamma_r), eval_f = logvraissemblance_pour_nloptr, opts = list(algorithm = "NLOPT_LN_BOBYQA", maxeval = 10000, tol_rel=1e-15, xtol_abs=1e-15), lb = c(1, 1, 0.1, 0, 0.01, 0))
  ...
  if(!(inherits(t, "try-error"))){
    ...
    Lvc <- c(Lvc, logvraissemblance(c(mu_r, sigma_r, lambda_r, alpha_r, beta_r, gamma_r))) #Pour avoir autant de points que les paramètres
    ma_liste <- list(logvraisemblance  = Lvc, lambda = suite_lambda, mu = suite_mu, sigma = suite_sigma, pi1 = suite_pi1, pi2 = suite_pi2, pi3 = suite_pi3, pi4 = suite_pi4, alpha = suite_alpha, beta = suite_beta, gamma = suite_gamma)
  }
  return (ma_liste)
}
toc()
...
summary(resultats)
saveRDS(resultats, file = "resultats_application_gev.RData")
################################################################################
##--------------------------------RESULTATS-----------------------------------##
################################################################################

resultats <- readRDS("resultats_application_gev.RData")
...
## gamma
resultats[[max_index]][[11]]
beta <- ggplot(as.data.frame(resultats[[max_index]][[11]]), aes(x=seq_along(resultats[[max_index]][[11]]), y=resultats[[max_index]][[11]])) +
  geom_point(color = "#21ADE5", size = 2.5) +
  theme_bw() +
  xlab("Itération") +
...
gamma_hat <- tail(resultats[[max_index]][[11]], 1)
...
logphi4 <- dgev(data, loc = alpha_hat, scale = beta_hat, shape = gamma_hat, log = TRUE)
