...
logvraissemblance <- function(params){
  ...
  logphi4 <- dlogcauchy(data, mu = params[4], sigma = params[5], log = TRUE)
...
resultats <- foreach (i=1:nbre_random_start, .packages=c("DPQ", "nloptr", "VaRES")) %dopar% {
  ...
  t <- try(repeat{
    ...
    logphi4 <- dlogcauchy(data, mu = alpha_r, sigma = beta_r, log = TRUE)
...
summary(resultats)
saveRDS(resultats, file = "resultats_application_logcauchy.RData")
################################################################################
##--------------------------------RESULTATS-----------------------------------##
################################################################################

resultats <- readRDS("resultats_application_logcauchy.RData")
...
proportion_25$poisson <- exp(dpois(1:25, lambda_hat, log = TRUE)  - logspace.sub(0, -lambda_hat))
...
plot_poisson_nbinom <- ggplot(proportion_25, aes(x = nb_transcrit)) +
  ...
  stat_function(aes(color= "orange"),size = 1, fun = function(x) {(dlogcauchy(x, mu = alpha_hat, sigma = beta_hat)*pi4_hat)})+
  ylab("Proportion") +
...
logphi4 <- dlogcauchy(data, mu = alpha_hat, sigma = beta_hat, log = TRUE)
