...
# une loi de Poisson, une loi binomiale négative et deux lois normales.
...
logvraissemblance <- function(params){
  ...
  logphi4 <- dnbinom(data, size = params[4], prob = params[5], log = TRUE) - logspace.sub(0, pnbinom(0, size = params[4], prob = params[5], log.p = TRUE))
...
resultats <- foreach (i=1:nbre_random_start, .packages=c("DPQ", "nloptr")) %dopar% {
  ...
  t <- try(repeat{
    ...
    logphi4 <- dnbinom(data, size = alpha_r, prob = beta_r, log = TRUE) - logspace.sub(0, pnbinom(0, size = alpha_r, prob = beta_r, log.p = TRUE))
...
## On arrête la parallélisation
stopCluster(nbre_coeurs_voulu)

## On sauvegarde le résultat
saveRDS(resultats, file = "resultats_application_nbinom_tronquée.RData")

################################################################################
##--------------------------------RESULTATS-----------------------------------##
################################################################################

resultats <- readRDS("resultats_application_nbinom_tronquee.RData")
...
## Graphique des groupes 2 et 4.

# On calcul les proportions à la main, ainsi que les valeurs de la fonction de masse de Poisson et de la binomiale négative.
...
graph_groupe_2_4 <- ggplot(proportion_25, aes(x = nb_transcrit)) +
  ...
  geom_point(aes(y = nbinom*pi4_hat, color = "Fonction de masse de la loi Négative Binomiale"), size=1.5) +
  ylab("Proportion") +
  xlab("Nombre de transcrits") +
  scale_color_manual(values = c("Fonction de masse de la loi de Poisson" = "hotpink1", "Fonction de masse de la loi Négative Binomiale" = "orange"))+
...
logphi4 <- dnbinom(data, size = alpha_hat, prob = beta_hat, log = TRUE) - logspace.sub(0, pnbinom(0, size = alpha_hat, prob = beta_hat, log.p = TRUE))
