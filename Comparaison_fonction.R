...
## Fonction de log-vraisemblance cmpltétée
...
## nloptr minimise, donc on minimise -log-vraisemblance complétée
...
pourcentage_des_coeurs_voulu <- 20/nbre_coeurs_disponibles
...
## Fonction de l'algorithme EM
boucle_sur_algos <- function(taile_ech, nbre_repet){
  for (k in 1:length(algorithmes)){
    ...
    for (l in 1:nbre_random_start){
      ...
      if(!(inherits(t, "try-error"))){
        ...
        resultats[1, taile_ech, nbre_repet, k, l] <- logvraissemblance(c(mu_r, sigma_r)) ## log-vraisemblance complétée
        resultats[2, taile_ech, nbre_repet, k, l] <- mean(temps_nlopt) ## On prend la moyenne de tous les nlopt utilisés lors de cet algorithme EM
        resultats[3, taile_ech, nbre_repet, k, l] <- mean(iteration_nlopt) ## On doit encore prendre la moyenne du nombre d'itération de nlopt lors de l'algorithme EM
        resultats[4, taile_ech, nbre_repet, k, l] <- lambda_r
        resultats[5, taile_ech, nbre_repet, k, l] <- mu_r
        resultats[6, taile_ech, nbre_repet, k, l] <- sigma_r
        resultats[7, taile_ech, nbre_repet, k, l] <- pi1_r
        resultats[8, taile_ech, nbre_repet, k, l] <- pi2_r
        resultats[9, taile_ech, nbre_repet, k, l] <- pi3_r
      }
...
for (i in 1:length(taille_echantillon)) {
  ...
  ## Création d'un tableau contenant tous les échantillons aléatoires (nécessaire? pour pouvoir faire du parralélisme)
  taille_echantillon_en_cours <- taille_echantillon[i]
  echantillons <- array(data = NA, c(nbre_repetition, taille_echantillon_en_cours))
  for (m in 1:nbre_repetition) {
    echantillons[m,] <- c(rpois(taille_echantillon_en_cours/2, lambda_cible), round(rnorm(taille_echantillon_en_cours/4, mu_cible, sigma_cible)), round(rnorm(taille_echantillon_en_cours/4, 2*mu_cible, sqrt(2)*sigma_cible)))
  }
  
  foreach (j=1:nbre_repetition) %dopar% {
    
    data <- echantillons[j,]
    boucle_sur_algos(taille_echantillon_en_cours, j)
