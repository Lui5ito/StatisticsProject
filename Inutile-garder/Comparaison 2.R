...
nbre_de_coeurs_voulu <- 30  ## Il faut que ca soit inférieur au nombre de coeurs disponibles
...
saveRDS(resultats_finaux, "resultats_comparaison2.RData")

################################################################################
##--------------------------------RESULTATS-----------------------------------##
################################################################################

resultats_finaux <- readRDS("resultats_comparaison2.RData")
...
plot_temps <- ggplot(data = for_boxplot) +
  ...
  labs(title = "Etude de la performance des algorithmes d'optimisation",
       ...
       y = "Temps de convergence en seconde (échelle logarithmique)") +
  ...
  theme_bw()
plot_temps
ggsave(plot=plot_temps, filename="performance_algos.png", width=9, height=6)
