...
# une loi de Pareto et deux lois normales.
...
melange_groupe_2 <- function(x){
  pi2_hat/(pi2_hat+pi3_hat)*dnorm(x, mean = mu_hat, sd = sigma_hat)
}

melange_groupe_3 <- function(x){
  pi3_hat/(pi2_hat+pi3_hat)*dnorm(x, mean = 2*mu_hat, sd = sqrt(2)*sigma_hat)
}

graph_groupes_2_3 <- ggplot(sum_gene_df %>% filter(nb_transcrit > 100)%>%filter(nb_transcrit < 50000), aes(x = nb_transcrit))+
