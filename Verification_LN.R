...
logvraissemblance <- function(params){
  ...
  logphi2 <- logspace.sub(pnorm(log(data+1/2), mean = params[1], sd = params[2], log.p = TRUE), pnorm(log(data-1/2), mean = params[1], sd = params[2], log.p = TRUE))
  logphi3 <- logspace.sub(pnorm(log(data+1/2), mean = log(2)+params[1], sd = params[2], log.p = TRUE), pnorm(log(data-1/2), mean = log(2)+params[1], sd = params[2], log.p = TRUE))
...
all_results_temp <-foreach (j=1:nbre_repetition, .packages = c("DPQ", "nloptr")) %dopar% {
  ...
  for (i in 1:length(taille_echantillon)) {
    ...
    data <- c(rpois(taille_echantillon_en_cours*0.6, lambda_cible), round(rlnorm(taille_echantillon_en_cours*0.1, mu_cible, sigma_cible)), round(rlnorm(taille_echantillon_en_cours*0.3, log(2)+mu_cible, sigma_cible)))
    ...
    for (k in 1:length(algorithmes)){
      ...
      for (l in 1:nbre_random_start){
        ...
        t <- try(repeat{
          ...
          logphi1 <- dpois(data, lambda_r, log = TRUE)
          logphi2 <- logspace.sub(pnorm(log(data+1/2), mean = mu_r, sd = sigma_r, log.p = TRUE), pnorm(log(data-1/2), mean = mu_r, sd = sigma_r, log.p = TRUE))
          logphi3 <- logspace.sub(pnorm(log(data+1/2), mean = log(2)+mu_r, sd = sigma_r, log.p = TRUE), pnorm(log(data-1/2), mean = log(2)+mu_r, sd = sigma_r, log.p = TRUE))
